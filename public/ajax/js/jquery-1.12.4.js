...
（功能（全局，工厂）{
	...
	if（typeof module ===“ object” && typeof module.exports ===“ object”）{
		//适用于CommonJS和类似CommonJS的环境，其中有适当的“窗口”
		//存在，执行工厂并获取jQuery。
		//对于没有带有文档的窗口的环境
...
//支持：Firefox 18+
//不能处于严格模式，包括ASP.NET trace在内的多个库
...
jQuery.extend（{
	...
	//从1.3版开始，DOM方法和函数（例如alert）
	//不支持。他们在IE（＃2968）上返回false。
	isFunction：function（obj）{
		返回jQuery.type（obj）===“函数”;
	}，
	...
	isNumeric：function（obj）{
		...
		//将无穷大减去NaN
		//加1可纠正parseFloat造成的精度损失（＃15100）
		var realStringObj = obj && obj.toString（）;
		返回！jQuery.isArray（obj）&&（realStringObj-parseFloat（realStringObj）+ 1）> = 0;
	}，
	...
	合并：功能（第一，第二）{
		...
		//支持：IE <9
		//在其他类似数组的对象（例如，NodeList）上将.length强制转换为NaN的解决方法
		如果（len！== len）{
...
//由于未在ES5中定义符号，因此JSHint将在此代码上出错。
//在.jshintrc中定义此全局变量会产生使用全局变量的危险
//在另一个地方不受保护，只为这些禁用JSHint似乎更安全
//三行。
/ * jshint忽略：开始* /
...
//填充class2type映射
jQuery.each（“布尔数字符串函数数组日期RegExp对象错误符号” .split（“”），
函数（i，名称）{
...
var i，
	...
	布尔值=“已选中|已选择|异步|自动对焦|自动播放|控件|已延迟|已禁用|隐藏|扭曲|循环|打开|只读|要求|范围”，
	...
	matchExpr = {
		...
		“ needsContext”：新的RegExp（“ ^” +空格+“ * [> +?] |：:( even | odd | eq | gt | lt | nth | first | last）（？：\\（” +
			空格+“ *（（？：-\\ d）？\\ d *）” +空格+“ * \\）|）（？= [^-] | $）”，“ i”）
	}，
	...
	如果（！seed）{
		...
		如果（documentIsHTML）{
			...
			如果（support.qsa &&
				...
				// qSA不在Element上下文之外，这不是我们想要的
				//感谢Andrew Dupont的这项变通方法
				//支持：IE <= 8
				//排除对象元素
				} if if（context.nodeName.toLowerCase（）！==“ object”）{
					...
					组= tokenize（选择器）;
					i = groups.length;
					nidselect = ridentifier.test（nid）吗？“＃” + nid：“ [id ='” + nid +“']”;
					当我 -  ） {
 ...
 *属性名称（带有空格的字符串）和（如果缓存大于Expr.cacheLength）
 *删除最早的条目
 * /
函数createCache（）{
	var keys = [];
...
isXML = Sizzle.isXML = function（elem）{
	//对documentElement进行验证，以了解尚不存在的情况
	//（例如在IE中加载iframe-＃4833）
...
setDocument = Sizzle.setDocument = function（node）{
	...
	如果（（parent = document.defaultView）&& parent.top！== parent）{
		//支持：IE 11
	...
	//因此请使用环形交叉路口getElementsByName测试
	...
	如果（（support.qsa = rnative.test（document.querySelectorAll）））{
		...
		assert（function（div）{
			...
			如果（div.querySelectorAll（“ [msallowcapture ^ ='']”）。length）{
				rbuggyQSA.push（“ [* ^ $] =” +空格+“ *（?:''| \” \“）”）;
			}
			...
			如果（！div.querySelectorAll（“ [id?=” + expando +“-]”）.length）{
				rbuggyQSA.push（“?=”）;
			}
			...
			//页内“ selector＃id sibing-combinator选择器”失败
...
Expr = Sizzle.selectors = {
	...
	preFilter：{
		...
		“ PSEUDO”：function（match）{
			...
			//去除未加引号的参数中多余的字符
				...
				//前进到下一个右括号
				（多余= unquoted.indexOf（“）”，unquoted.length-多余）-unquoted.length））{
	...
	假人：{
		...
		“ checked”：function（elem）{
			...
			return（nodeName ===“ input” && !! elem.checked）|| （nodeName ===“选项” && !! elem.selected）;
		}，
...
//添加按钮/输入类型伪
for（我在{radio：true，复选框：true，file：true，password：true，image：true}）{
			...
			如果（xml）{
				...
				while（（elem = elem [dir]））{
					如果（elem.nodeType === 1 || checkNonElements）{
						...
						如果（（oldCache = uniqueCache [dir]）&&
							oldCache [0] === dirruns && oldCache [1] === doneName）{

							//分配给newCache，以使结果反向传播到先前的元素
							return（newCache [2] = oldCache [2]）;
						}其他{
							//重用newcache，以便将结果反向传播到先前的元素
							uniqueCache [dir] = newCache;

							//匹配表示我们已经完成；失败意味着我们必须继续检查
...
函数setMatcher（preFilter，selector，matcher，postFilter，postFinder，postSelector）{
	...
	返回markFunction（function（种子，结果，上下文，xml）{
		var temp，i，elem，
			...
			matcherOut =匹配器？
				//如果我们有postFinder，已过滤的种子或非种子的postFilter或预先存在的结果，
		...
		如果（种子）{
			如果（postFinder || preFilter）{
				如果（postFinder）{
					//通过将该中间语言压缩到postFinder上下文中来获取最终的matcherOut
					temp = [];
					i = matcherOut.length;
					当我 -  ） {
 ...
 *选择器功能
 * @param {String | Function}选择器选择器或预编译
...
select = Sizzle.select = function（选择器，上下文，结果，种子）{
	...
	//如果没有提供，则编译并执行过滤功能
	//如果我们修改了上面的选择器，则提供`match`以避免重新令牌化
	（编译||编译（选择器，匹配））（
		种子，
		上下文
		！documentIsHTML，
...
函数winnow（elements，qualifier，not）{
	...
	返回jQuery.grep（elements，function（elem）{
		return（jQuery.inArray（elem，qualifier）> -1）！==不;
	}）；
}
